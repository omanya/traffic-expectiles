...
bibliography: [references.bib, weather-expectiles.bib] 
---
...
\end{equation}
This introduces an asymmetric *quadratic* loss on early or late arrival. In this preference formulation, the marginal disutility *increases linearly with the delay magnitude*, potentially describing the preferences of at least some commuters more consistently. Notably, this *continuously differentiable* preference specification fulfills the assumptions of @smith1984 and @daganzo1985 required for the existence and uniqueness of equilibrium in a single bottleneck model. It also satisfies the requirements of @depalma2012 and their equilibrium analysis under risk aversion and information value.

Both preference specifications in \@ref(eq:u1) and \@ref(eq:u2) share the property that the optimal departure time choices correspond to specific *tail indices* of the travel time distribution. For the first specification in \@ref(eq:u1), these are the *quantiles*, and for the second specification in \@ref(eq:u2), these are the *expectiles* of the travel time. The asymmetric quadratic version of the scheduling preferences, and the fact that the solutions to the expected utility maximization problem are quantiles and expectiles (and in general the so-called $L_p$-quantiles) as *the minimizers of expectations in asymmetric* $L_p$*-norm*, seem to have been largely unaddressed explicitly in the previous literature. Moreover, both types of the scheduling preferences are related via the quantile-expectile correspondence (@jones1994), with the second version in \@ref(eq:u2) describing a class of preferences with more favorable properties to the first, as demonstrated below.

In this paper, we analyse and compare the two utility specifications of scheduling preferences: the linear one in \@ref(eq:u1) and the quadratic one in \@ref(eq:u2), with the latter offers advantages over the former. First, we review the existing specification and formally define the new asymmetric quadratic utility specification in Section 2. In Section 3, we introduce the concepts of quantiles and expectiles as minimizers of the expectations of the respective asymmetric $L_p$-norms, and show that the optimal departure times are the quantiles of $T$ for the linear ($p=1$) and the expectiles of $T$ for the quadratic ($p=2$) case. Furthermore, we link the optimal choices derived from the two utility specifications through the concept of quantile-expectile correspondence and demonstrate that the new asymmetric quadratic class is valid alternative, offering conceptional and computational advantages over the linear asymmetric case in \@ref(eq:u1).  We also derive the travel time reliability ratios to quantify travel time reliability for both specifications and propose their estimation using the related concepts of \(\tau\)-deviation and \(\tau\)-variance to complete Section 3.  Using the travel time data of Montreal, we compare in Section 4 the optimal departure time choices for two routes corresponding to the two utility specifications and the resulting travel time reliability ratios based on a fitted mixture of gamma distributions with two components representing the free-flow and congestion regimes. Finally, we conclude and discuss the results.
...
\begin{equation}q_\tau = \inf(x: F(x)\geq \tau),(\#eq:inf)
\end{equation}
...
```{r figcor, out.width="100%",  fig.height = 3, fig.width = 10,fig.cap="The quantile-expectile correspondence for the mixture of two uniform distributions in Example 3.4 for various quantile and expectile levels and $p=0.8$."}
par(mar=c(2.6,2.5,2,2), mfrow=c(1,2))
par(xpd=T)
...
for(i in c(3:9,11)){
  ...
  text(omega[i], 0,round(omega[i],2),col=2,pos=1, cex=0.5)
  text(0,tomega[i], round(tomega[i],2),col=4,pos=2,cex=0.5)
}
i=2
 arrows(omega[i],0,omega[i],tomega[i],lty=2, col=2,code=0)
  arrows(0,tomega[i],omega[i],tomega[i],lty=2, col=4,code=0)
  ...
  text(omega[i], 0,round(omega[i],2),col=2,pos=1, cex=0.5)
  text(-0.05,tomega[i]+0.01, round(tomega[i],2),col=4,pos=2,cex=0.5)

i=12
 arrows(omega[i],0,omega[i],tomega[i],lty=2, col=2,code=0)
  arrows(0,tomega[i],omega[i],tomega[i],lty=2, col=4,code=0)
  
  text(omega[i]+0.01, 0.04,round(omega[i],2),col=2,pos=1, cex=0.5)
  text(-0.05,tomega[i]+0.01,round(tomega[i],4),col=4,pos=2,cex=0.5)

par(xpd=T)
arrows(p0,-0.05,p0,1,col="gray44",code=0,lty=2)
#
text(p0-0.2,0.7,bquote(omega==p),col="gray44",pos=3)
arrows(0.7,0.72,0.79,0.5,code=2,length=0.1,col="gray44")
#
#points(w, tw)
...
arrows(0,-0.1,0,1.1,code=2,length=0.1)
text(c(0,0,0),c(-0.07,0.5,1.05),c(0,0.5,1),pos=2)
...
for(i in c(1:6,12)){
  ...
  text(0,tau_new$x[i], round(tau_new$x[i],2),col=4,pos=2, cex=0.5)
  text(tau_new$y[i], 0,round(tau_new$y[i],2),col=2,pos=1,cex=0.5)
}
i=13
 arrows(0,tau_new$x[i],tau_new$y[i],tau_new$x[i],lty=2, col=4,code=0)
  arrows(tau_new$y[i],0,tau_new$y[i],tau_new$x[i],lty=2, col=2,code=0)
  text(0,tau_new$x[i], round(tau_new$x[i],2),col=4,pos=2, cex=0.5)
  text(tau_new$y[i], 0.04,round(tau_new$y[i],2),col=2,pos=1,cex=0.5)
...
Let $U_a(d,T), a\in\{1,2\}$ specified in \@ref(eq:ua) be the traveler's utility function with $\beta_a>0$ and $\gamma_a>0$ fixed. Let  $T$ be a random travel time with distribution function $F$. Then, optimal departure time $d_a^*$ corresponds to:
...
where $h_{a,\tau} = \begin{cases} q_\tau(T),&\text{for } a=1,\\ e_\tau(T),&\text{for }a=2,\end{cases}$ and $\tau = \frac{\gamma}{\beta+\gamma}$.
:::
...
```{r}
# links<-unique(dataf$LinkId)
# i=which(links=="LND_1E-1B")#174  #174 36  32 23 157
# i2=which(links=="LSh_01-40")#173 # reciprocal direction to 174
# #i3=which(links=="LPa_19-18")#which(links=="LSM_13-14")
# hrs<-c("06","08")#6-8, 6-9
# ind<-dataf$LinkId==links[i]&dataf$wd%in%c("Mo","Di","Mi", "Do")&dataf$hour%in%hrs#&dataf$month%in%c("Feb")
# ind2<-dataf$LinkId==links[i2]&dataf$wd%in%c("Mo","Di","Mi", "Do")&dataf$hour%in%hrs#&dataf$month%in%c("Feb")
# #ind3<-dataf$LinkId==links[i3]&dataf$wd%in%c("Mo","Di","Mi", "Do")&dataf$hour%in%hrs#&dataf$month%in%c("Feb")
# par(mfrow=c(1,2))
# #minn<-min(sum(ind3), sum(ind2))
# #hist(dataf$TravelTime_s[ind2][1:minn]+dataf$TravelTime_s[ind3][1:minn])
# 
# tt2<-(dataf$TravelTime_s[ind2])/60#[1:minn]+dataf$TravelTime_s[ind2][1:minn])/60#datafs$TravelTime_s/60
# tt<-dataf$TravelTime_s[ind]/60
# ###################################
...
# plot(den$x,den$y,ylim=c(-0.01,0.50),axes=F, xlim=c(0,50), main="", xlab="travel time",ylab="density estimate", type="l")
# lines(den2$x, den2$y, col="orange")
# points(tt, rep(-0.01,length(tt)),pch="|",col=scales::alpha("gray44",0.5))
# par(xpd=T)
...
```{r}
# # #use mixtools to fit mix of gamma: https://stackoverflow.com/questions/75911776/fitting-gamma-mixture-distribution-in-r
# library(mixtools)
# set.seed(123)
# mixmod <- gammamixEM(tt, k=2, maxit=10000)
# 
# mixmod2 <- gammamixEM(tt2, k=2, maxit=5000)
# 
# 
# save(mixmod,file="mixmod.RData")
# save(mixmod2,file="mixmod2.RData")
```
...
```{r tabpars}
tbl<-round(rbind(c(mixmod$lambda[2],mixmod$gamma.pars[1,2],mixmod$gamma.pars[2,2],
...
colnames(tbl)<-c("$p_{1,.}$","$k_{1,.}$","$\\theta_{1,.}$","$p_{2,.}$","$k_{2,.}$","$\\theta_{2,.}$")

caption<-paste0("The estimated parameters of the four-component gamma mixture model (sample sizes $n_{1}=",length(tt), "$ and $n_{2}=", length(tt2),"$).")
...
Both models assign similar probabilities for the free-flow regime, captured by the first component (`r sum(tbl[1,1])` and `r sum(tbl[1,4])` respectively), and the congestion regime (`r sum(tbl[2,1])` and `r sum(tbl[2,4])` respectively). Note that, although  the second model is associated with a slightly higher probability of the free-flow regime (`r tbl[1,4]` compared to `r tbl[1,1]`),  it also reveals higher means and variances in both regimes ($k_{1,2}\cdot \theta_{1,2}=`r sum(tbl[1,5]*tbl[1,6])`$ compared to $k_{1,1}\cdot \theta_{1,1}=`r sum(tbl[1,2]*tbl[1,3])`$ and $k_{1,2}\cdot \theta_{1,2}^2=`r sum(tbl[1,5]*tbl[1,6]^2)`$ compared to $k_{1,1}\cdot \theta_{1,1}^2=`r sum(tbl[1,2]*tbl[1,3]^2)`$). Therefore, the more conservative choice would be route 1. In the following analysis, we examine the choices based on the proposed utility formulations.
...
The corresponding optimal departure time choices for both utility types are plotted in Figure \@ref(fig:choices). On the right panel of the figure, we show the estimated optimal choices as quantiles for $U_1$ and expectiles for $U_2$, varying the preference parameter $\tau=\frac{\gamma}{\beta+\gamma}$. We estimate the quantiles and the expectiles based on the fitted mixture distributions using Monta-Carlo approximation and the formulas in \@ref(eq:q) and \@ref(eq:e). The computations are performed in R, utilizing the *quantile* function from the *rbase* and the *expectile* function  from the *expectreg* package to obtain the empirical counterparts.
...
Finally, we use the travel time data from Montreal to fit a mixture of gamma components model and empirically demonstrate how the optimal departure choices induced by both utility specifications evolve with varying preference parameters. We estimate the quantile-expectile correspondence for the data, compare the resulting travel time reliability ratios, and highlight the advantages of the asymmetric quadratic utility specification, making it a valid alternative to the classical scheduling utility. This approach provides potential advantages for modeling various risk perceptions in transportation networks.
